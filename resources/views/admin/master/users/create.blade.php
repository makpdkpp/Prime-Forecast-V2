...
@section('title', 'เพิ่มผู้ใช้งานใหม่ | PrimeForecast')
        ...
        <div class="card-body">
            <form action="{{ route('admin.users.store') }}" method="POST">
                ...
                <div class="alert alert-info">
                    <i class="fas fa-info-circle"></i> ระบบจะส่งอีเมลเชิญไปยังผู้ใช้ใหม่ เพื่อให้ตั้งรหัสผ่านเอง
                </div>
                <div class="row">
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="nname" id="nname" class="form-control @error('nname') is-invalid @enderror" value="{{ old('nname') }}" required>
                            @error('nname')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" name="surename" id="surename" class="form-control @error('surename') is-invalid @enderror" value="{{ old('surename') }}" required>
                            @error('surename')
                                <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
                                ...
                                @foreach($teams as $team)
                                    <div class="col-md-3">
                                        <div class="custom-control custom-checkbox">
                                            <input type="checkbox" class="custom-control-input" id="team_{{ $team->team_id }}" name="teams[]" value="{{ $team->team_id }}" {{ in_array($team->team_id, old('teams', [])) ? 'checked' : '' }}>
